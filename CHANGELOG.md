...
- **Traceroute (Windows)**: Capped the per-probe wait to 1s (`-w 1000`/`-w 1`) instead of relying on `tracert`'s 4s default; with 3 probes/hop and several non-responding hops beyond the local network, a 5-hop trace could take over a minute and blow past CI's timeout. Also added an explicit subprocess timeout so a hung trace fails fast instead of hanging the whole job
